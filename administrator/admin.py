...
class ReservationRoomInline(admin.TabularInline):
    ...
    extra = 0
    autocomplete_fields = ('room', 'key_given_by', 'key_returned_by')
...
class ReservationAdmin(admin.ModelAdmin):
    ...
    inlines = (ReservationRoomInline, StayRecordInline)


@admin.register(ReservationRoom)
class ReservationRoomAdmin(admin.ModelAdmin):
    list_display = ('reservation', 'room', 'price_per_night', 'key_status', 'key_given_at', 'key_returned_at')
    list_filter = ('key_status',)
    search_fields = ('reservation__code', 'room__number', 'room__name')
    autocomplete_fields = ('reservation', 'room', 'key_given_by', 'key_returned_by')


@admin.register(StayRecord)
class StayRecordAdmin(admin.ModelAdmin):
    list_display = ('reservation', 'ktp_status', 'ktp_received_at', 'ktp_returned_at')
    list_filter = ('ktp_status',)
    search_fields = ('reservation__code', 'reservation__customer__username')
    autocomplete_fields = ('reservation', 'ktp_received_by', 'ktp_returned_by')
