...
class Room(models.Model):
    ...
    room_type = models.ForeignKey(RoomType, on_delete=models.PROTECT, related_name='rooms', verbose_name='Tipe')
    custom_price = models.DecimalField('Harga khusus', max_digits=12, decimal_places=2, blank=True, null=True)
    description = models.TextField('Deskripsi', blank=True)
    facilities = models.ManyToManyField(Facility, blank=True, related_name='rooms', verbose_name='Fasilitas')
    ...
    def is_available(self, check_in=None, check_out=None, exclude_reservation=None):
        ...
        qs = self.reservations.exclude(status__in=[Reservation.Status.CANCELED, Reservation.Status.COMPLETED])
        if exclude_reservation:
            qs = qs.exclude(pk=exclude_reservation)
        return not qs.filter(check_in__lt=check_out, check_out__gt=check_in).exists()
...
class Reservation(models.Model):
    ...
    customer = models.ForeignKey(User, on_delete=models.PROTECT, related_name='hotel_reservations', verbose_name='Customer')
    room = models.ForeignKey(Room, on_delete=models.PROTECT, related_name='reservations', verbose_name='Kamar')
    ...
    status = models.CharField('Status', max_length=30, choices=Status.choices, default=Status.WAITING_PAYMENT)
    source = models.CharField('Sumber reservasi', max_length=20, choices=Source.choices, default=Source.ONLINE)
    ...
    def clean(self):
        ...
        if self.room_id and self.guests and self.guests > self.room.room_type.capacity:
            errors['guests'] = f'Maksimal {self.room.room_type.capacity} tamu untuk kamar ini.'
        if self.room_id and self.check_in and self.check_out and self.check_out > self.check_in:
            if not self.room.is_available(self.check_in, self.check_out, self.pk):
                errors['room'] = 'Kamar tidak tersedia pada rentang tanggal tersebut.'
        if errors:
...
class Payment(models.Model):
    ...
    reservation = models.OneToOneField(Reservation, on_delete=models.CASCADE, related_name='payment', verbose_name='Reservasi')
    ...
    status = models.CharField('Status pembayaran', max_length=20, choices=Status.choices, default=Status.PENDING)
...
class HotelSetting(models.Model):
    ...
    tagline = models.CharField('Tagline', max_length=200, default='A timeless stay, wrapped in comfort.')
    description = models.TextField('Tentang hotel', default='RoseHaven adalah hotel bintang 4 dengan pelayanan hangat, kamar elegan, dan fasilitas lengkap.')
