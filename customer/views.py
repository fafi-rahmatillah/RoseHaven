...
from administrator.models import HotelSetting,Payment,Reservation,ReservationRoom,Room
...
from rosehaven.reservations_services import attach_booked_periods, sync_reservation_rooms
...
def contact(request):
    if request.method == "POST":
        messages.success(
            request,
            "Pesan Anda telah diterima. Tim RoseHaven akan segera menghubungi Anda.",
        )
    ...
    if request.method == "POST" and form.is_valid():
        with transaction.atomic():
            ...
            CustomerProfile.objects.create(
                ...
                marriage_certificate=form.cleaned_data.get(
            ...
            messages.success(
                request,
                f"Registrasi berhasil. Username Anda: {username}",
            )
...
def reserve(request, room_id=None):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        messages.success(
            request,
            f'Reservasi {reservation.code} berhasil dibuat. '
            'Silakan unggah bukti pembayaran. '
            'KTP asli akan diperiksa resepsionis dan '
            'dititipkan sebagai jaminan sebelum check-in.',
        )

        return redirect(
            'customer:payment_upload',
            pk=reservation.pk,
        )
...
def payment_upload(request, pk):
    ...
    if reservation.status in [
        ...
        messages.error(
            request,
            "Reservasi ini tidak dapat dibayar."
        )
        return redirect(
            "customer:reservation_detail",
            pk=pk,
        )
    ...
    if request.method == "POST" and form.is_valid():
        ...
        messages.success(
            request,
            "Bukti pembayaran berhasil diunggah dan menunggu verifikasi petugas."
        )
...
def reservation_detail(request, pk):
    reservation = get_object_or_404(
        Reservation.objects.prefetch_related(
            "reserved_rooms__room__room_type"
        ),
        pk=pk,
        customer=request.user,
    )
...
def cancel_reservation(request, pk):
    ...
    if request.method == "POST":
        ...
        if reservation.status == Reservation.Status.WAITING_PAYMENT:
            ...
            messages.error(
                request,
                "Reservasi hanya dapat dibatalkan sebelum pembayaran dikonfirmasi."
            )
