...
class CustomerFlowTests(TestCase):
    ...
    def reservation_payload(self, check_in=None, check_out=None):
        check_in = check_in or timezone.localdate() + timedelta(days=3)
    ...
    def test_registration_creates_customer_group_and_profile(self):
        ...
        self.assertEqual(user.customer_profile.nik, '3372015201010002')
        self.assertEqual(
            user.customer_profile.identity_status,
            CustomerProfile.IdentityStatus.PENDING,
        )

    def test_pending_identity_can_create_reservation(self):
        """Validasi KTP tidak boleh menjadi syarat pembuatan reservasi."""
        self.client.login(username='cust', password='StrongPass123!')
        response = self.client.post(
            reverse('customer:reserve_room', args=[self.room.pk]),
            self.reservation_payload(),
        )
        reservation = Reservation.objects.get(customer=self.user)
        self.assertRedirects(
            response,
            reverse('customer:payment_upload', args=[reservation.pk]),
        )
        self.assertEqual(reservation.status, Reservation.Status.WAITING_PAYMENT)
        self.assertEqual(reservation.reserved_rooms.count(), 1)

    def test_rejected_identity_can_still_create_reservation(self):
        """Penolakan identitas menahan check-in, bukan pemesanan kamar."""
        self.profile.identity_status = CustomerProfile.IdentityStatus.REJECTED
    ...
    def test_reservation_and_payment_upload(self):
        ...
        self.assertRedirects(
            response,
            reverse('customer:reservation_detail', args=[reservation.pk]),
        )
    ...
    def test_overlapping_reservation_rejected(self):
        ...
        self.assertContains(response, 'sudah dipesan pada tanggal tersebut')
