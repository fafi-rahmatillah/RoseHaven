...
def sync_reservation_rooms(reservation, rooms):
    """
    Simpan seluruh kamar dalam satu reservasi
    dan hitung ulang total pembayaran.
    """
    ...
    reservation.reserved_rooms.exclude(
        room_id__in=selected_ids
    ).delete()
...
def attach_booked_periods(rooms):
    """
    Menambahkan booked_periods pada setiap kamar
    untuk ditampilkan kepada customer.
    """
    ...
    reservations = (
        Reservation.objects
        .filter(
            Q(room_id__in=room_map)
            | Q(reserved_rooms__room_id__in=room_map),
    ...
    for reservation in reservations:
        room_ids = set(
            reservation.reserved_rooms.values_list(
                'room_id',
                flat=True,
            )
        )
    ...
    for room in rooms:
        room.booked_periods = periods.get(
            room.pk,
            [],
        )
